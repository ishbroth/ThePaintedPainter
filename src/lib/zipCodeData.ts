import { HIGH_COST_ZIP_PREFIXES } from './pricingConfig';
...
const ZIP_TO_STATE: Record<string, string> = {
  ...
  '540': 'MN', '541': 'MN',
  '543': 'WI', '544': 'WI', '545': 'WI', '546': 'WI', '547': 'WI',
  '548': 'WI', '549': 'WI',
  ...
  '640': 'KS', '641': 'MO',
  '644': 'MO', '645': 'MO', '646': 'MO', '647': 'MO', '648': 'MO',
  '649': 'MO', '650': 'MO', '651': 'MO', '652': 'MO', '653': 'MO',
...
// State compliance notes for painting contractors
const STATE_COMPLIANCE: Record<string, string[]> = {
  CA: [
    'California requires CSLB contractor license for jobs over $500',
    'CA EPA lead-safe certification required for pre-1978 homes',
    'VOC regulations: max 50 g/L for flat coatings, 100 g/L for non-flat',
  ],
  NY: [
    'NYC requires EPA RRP certification for pre-1978 work',
    'NYC Local Law 31: lead-safe work practices in child-occupied facilities',
  ],
  MA: [
    'Massachusetts requires lead-safe renovation for pre-1978 homes',
    'MA lead paint law is stricter than federal EPA RRP rule',
  ],
  IL: [
    'Illinois Lead Poisoning Prevention Act applies to pre-1978 homes',
    'Chicago requires city business license for painting contractors',
  ],
  TX: [
    'Texas does not require a state painting contractor license',
    'Local jurisdictions may require permits for exterior work',
  ],
  FL: [
    'Florida requires painting contractor registration',
    'Hurricane zones may have additional coating requirements',
  ],
  WA: [
    'Washington requires EPA RRP certification for pre-1978 work',
    'L&I registration required for painting contractors',
  ],
  PA: [
    'Pennsylvania requires EPA RRP certification for pre-1978 work',
    'Philadelphia requires city contractor license',
  ],
};
...
export function isHighCostArea(zip: string): boolean {
  const prefix = zip.substring(0, 3);
  return HIGH_COST_ZIP_PREFIXES.includes(prefix);
}

export function getStateComplianceNotes(state: string): string[] {
